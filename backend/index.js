...
app.get("/init", (req, res) => {
  const productJsonPath = "./product_transaction.json";
...
app.get("/transactions", async (req, res) => {
  ...
  const result = await res.json({ response: transactions });
});
...
app.get("/stats", async (req, res) => {
  ...
  if ("text" === type) {
    ...
    const pieResponse = await getPieStats(matchMonth);
    const result = await res.json({
      response: {
        textResponse: textResponse,
        barResponse: barResponse,
        pieResponse: pieResponse,
      },
    });
  }
});
